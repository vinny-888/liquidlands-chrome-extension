      ...
      chrome.storage.local.get("version",function(res) {
        ...
        loadJSONData().then(() => {
          getItems(res.version).then(items => {
            ...
            let filtered = items.filter((item)=>canBeBuilt(item));
...
async function buildItemComplete(name, cities, version){
  ...
  sortedByLevel.forEach((key, index)=>{
    let citiesWithItem2 = getCitiesWithItem(item.title);
    citiesWithItem2.sort((a,b) => (b.buildings[name].count > a.buildings[name].count) ? 1 : ((a.buildings[name].count > b.buildings[name].count) ? -1 : 0))
    let bestCity = citiesWithItem2[0];
    if(cities){
    ...
    let building = bestCity.buildings[key];
    let quantity = children[key].count;
    buildItems(bestCity.id, item.id, building.x, building.y, quantity, version)
  });
}
