...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    if (request.action === "myItems") {
    ...
    }   else if (request.action === "explorerItems") {
      console.log("Message received in background script");
      chrome.storage.local.get("version", async function(res) {
        console.log("Message received in background script with version:", res);
        let stats = await explorerItems(res.version);
      ...
      chrome.storage.local.get("version",function(res) {
        ...
        loadJSONData().then(() => {
          getItems(res.version).then(items => {
            console.log('Got items', items);
            let filtered = items.filter((item)=>canBeBuilt(item) && item.used_points > 0);
...
async function loadJSONData() {
  return new Promise(async (resolve, reject) => {
      try {
          ...
          all_items = convertJSON(jsonData);
          resolve(); // Resolve the promise after successful completion of all async tasks
      } catch (error) {
...
function convertJSON(json){
  ...
  json.forEach((item)=>{
      ...
      let children = [];
      children = getChildren(json, newItem, 1);
...
function canBeBuilt(item){
  ...
  Object.keys(children).forEach((key, index)=>{
      ...
      let child = all_items.find((item)=>item.title == key);
      if(child){
...
function getChildren(arr, item, level){
  ...
  if(item.input1){
      ...
      children1 = getChildren(arr, childItem, level+1);
  ...
  if(item.input2){
      ...
      let childItem = buildItem(child, null);
      childItem.level = level;
      children2 = getChildren(arr, childItem, level+1);
  ...
  if(item.input3){
      ...
      let childItem = buildItem(child, null);
      childItem.level = level;
      children3 = getChildren(arr, childItem, level+1);
...
function getSortedKeysByLevel(obj) {
  var keys = Object.keys(obj);
  ...
  for(let i=0; i< res1Json.d.cards.length; i++){
    ...
    if(bricks.length > 0){
      for(let j=0; j< bricks.length; j++){
        let brick = bricks[j];
        if(brick.attack > 0 && brick.defence == 0){
...
async function explorerItems(version){
  let res1 = await fetch("https://liquidlands.io/controller", {
    "headers": {
      "accept": "application/json, text/javascript, */*; q=0.01",
      "content-type": "application/json; charset=UTF-8",
  ...
  for(let i=0; i< res1Json.d.cards.length; i++){
    ...
    if(card.bricks_per_day > 3){
      stats.ranges['3.0'].count++;
      stats.ranges['3.0'].total+=card.defence;
    } else if(card.bricks_per_day > 2){
      stats.ranges['2.0'].count++;
      stats.ranges['2.0'].total+=card.defence;
    } else if(card.bricks_per_day > 1.5){
      stats.ranges['1.5'].count++;
      stats.ranges['1.5'].total+=card.defence;
    } else if(card.bricks_per_day > 1.0){
      stats.ranges['1.0'].count++;
      stats.ranges['1.0'].total+=card.defence;
    } else if(card.bricks_per_day > 0.7){
      stats.ranges['0.7'].count++;
      stats.ranges['0.7'].total+=card.defence;
    } else if(card.bricks_per_day > 0.5){
      stats.ranges['0.5'].count++;
      stats.ranges['0.5'].total+=card.defence;
    } else if(card.bricks_per_day > 0.4){
      stats.ranges['0.4'].count++;
      stats.ranges['0.4'].total+=card.defence;
    } else if(card.bricks_per_day > 0.3){
      stats.ranges['0.3'].count++;
      stats.ranges['0.3'].total+=card.defence;
    } else if(card.bricks_per_day > 0.2){
      stats.ranges['0.2'].count++;
      stats.ranges['0.2'].total+=card.defence;
    } else if(card.bricks_per_day > 0.1){
      stats.ranges['0.1'].count++;
      stats.ranges['0.1'].total+=card.defence;
    } else if(card.bricks_per_day > 0){
      stats.ranges['0.0'].count++;
      stats.ranges['0.0'].total+=card.defence;
    }
    ...
    bricks.forEach((brick)=>{
      stats.bricks.attack += brick.attack;
      stats.bricks.defense += brick.defence;
      stats.bricks.count += 1;
    })

    items.forEach((item)=>{
      stats.items.attack += item.attack;
      stats.items.defense += item.defence;
      stats.items.count += 1;
    })
...
async function buildItemComplete(name, cities, version){
  ...
  for(let i=0; i<sortedByLevel.length; i++){
    ...
    let building = bestChildCity.buildings[key];
  ...
  for(let i=0;i<quantity; i++){
    try{
      let response = await fetch("https://liquidlands.io/controller", {
        ...
        "body": "{\"controller\":\"Buildings.MakeItem\",\"fields\":{\"city_id\":"+city_id+",\"x\":"+x+",\"y\":"+y+",\"blueprint_id\":"+blueprint_id+",\"app_version\":\""+version+"\"},\"nonce\":"+Date.now()+",\"debug\":true}",
        "method": "POST",
        "credentials": "include"
      })
